...
from HanlerLineImage import HandlerLineImage
from scipy import stats

# resultImg = HandlerLineImage("test.JPG")
...
class ClusterPlot:
    def __init__(self, n_components: int = 2, anchors_method: str = 'birch', n_intra_anchors: int = None,
                 ...
                 transpose_low_dim: bool = False, point_margin = 0.01):
        """
        ...
        classes. 0 is convec hull, increasing the alpha will result in c.
        ...
        # 降维方法的选择 t-sne umap mds pca lda
        ...
        # 聚类方法 agglomerative kmeans birch
        ...
        # 检查以下参数必须大于0  k, proximity_k 分别是KNN算法中的K，用以评估Overlap和proximity.
    ...
    def fit_transform(self, X, y):
        ...
        # 此时的X,Y 为高维空间的数据 并且增加了Birch聚类算法聚类出来的anchors
    ...
    def _get_intra_class_anchors(self, X, y):
        ...
        #中间变量anchors_df 存储的是anchors的value 和 label
        #输出intra_class anchors, intra_class anchors_label
        anchors_dfs = []
    ...
    def _calc_high_dim_clusters_properties(self, df, label_col, cluster_col, intra_centroids_df):
        ...
        :param df: data pandas Dataframe   原始数据的df形式
        ...
        # df是原始高维数据
        df = df.copy()
        # intra_centroids_df 存了anchor点的高维坐标和标签
        ...
        # quantile表示算分位数,q = 1 表示分位数为最大值, 这里算出来各个子类的anchor的半径
    ...
    def _build_knng(self, X):
        ...
        :return: None
        """
        # self.k表示寻找的邻居数量,这里默认为20
        # n_jobs 是并行寻找近邻值的数量，设定为-1表示并行数量是CPU的内核数
    ...
    # WSC
    def _pebble_plot(self, line_width=2, alpha=0.2, douglas_peucker_tolerance=0.6, smooth_iter=3, figsize: tuple = (21, 15),):
        df = self.vis
        ...
        for label in sorted(contours_df[self.label_col].unique()):
            ...
            for concave_hull in concave_hulls:
                ...
                patch = patches.PathPatch(path, facecolor=c, alpha=alpha, linewidth=None, edgecolor=None)
                ax.add_patch(patch)
        ...
        if self.show_blobs:
            sns.scatterplot(data=df, x=self.x_col, y=self.y_col, hue=self.label_col,
    ...
    def _pebble_voronio_plot(self, line_width=2, alpha=0.2, douglas_peucker_tolerance=0.6, smooth_iter=3,
                     figsize: tuple = (21, 15), ):
        df = self.vis
        fig, ax = plt.subplots(figsize=figsize)
        hue_order = sorted(df[self.label_col].unique())
        ...
        for label in sorted(contours_df[self.label_col].unique()):
            ...
            for region in regions:
                ...
                for p in list(polygons):
                    ...
                    ax.add_patch(patch)
    # pebble with density
    def _pebble_plot_kdeplot(self,line_width=2, alpha=0.2, douglas_peucker_tolerance=0.6, smooth_iter=3, figsize: tuple = (21, 15),):
        # self._pebble_plot(self, line_width=line_width, alpha=alpha,
        #                   douglas_peucker_tolerance=douglas_peucker_tolerance,
        #                   smooth_iter=smooth_iter, figsize=figsize)
        df = self.vis
        fig, ax = plt.subplots(figsize=figsize)
        hue_order = sorted(df[self.label_col].unique())
        sns.lineplot(data=df[df['anchor'] == True], x=self.x_col, y=self.y_col, hue=self.label_col,
        ...
        # data1 = data[data['label'] == '0_Poly0']
        # data2 = data[data['label'] == '1_Poly1']
        m1 = np.array(data["x"])
        m2 = np.array(data["y"])
        xmax = m1.max()
        xmin = m1.min()
        ...
        sns.scatterplot(data=data, x=self.x_col, y=self.y_col, hue=self.label_col,
                        style=self.cluster_col if show_anchors else None, ax=ax,
                        alpha=alpha if show_blobs else 1, legend=False,
                        hue_order=hue_order, s=240)
    ...
    def _anchors_plot_sns_separate(self, i, figsize: tuple = (21, 15),
        ...
        if show_inner_blobs:
            self._points_anchors_patches_plot(df, i, show_inner_blobs=True, annotate_images=False, figsize=figsize,
                                              ...
                                              douglas_peucker_tolerance=douglas_peucker_tolerance,
                                              smooth_iter=smooth_iter)
            # self._pebble_plot_kdeplot(df, alpha=0)
    ...
    # visualization including blob、scatterplot、matrix
        ...
        if show_blobs:
            ...
            for label in sorted(contours_df[self.label_col].unique()):
                ...
                for concave_hull in concave_hulls:
                    ...
                    patch = patches.PathPatch(path, facecolor=c, alpha=0, linewidth=None, edgecolor=None)
                    ax.add_patch(patch)
                    # patch = patches.PathPatch(path, facecolor=None, linewidth=10, edgecolor=c, fill=False)
                    patch = patches.PathPatch(path, facecolor=None, linewidth=2, edgecolor=c, fill=False)
                    ax.add_patch(patch)
    ...
    def _get_contour_df(self):
        ...
        x_plus_arr[:, 0] = low_dim_arr[:, 0] + (self.anchors_radius if self.radius_q is not None else point_margin)
        x_minus_arr = low_dim_arr.copy()
        x_minus_arr[:, 0] = low_dim_arr[:, 0] - (self.anchors_radius if self.radius_q is not None else point_margin)
        y_plus_arr = low_dim_arr.copy()
        y_plus_arr[:, 1] = low_dim_arr[:, 1] + (self.anchors_radius if self.radius_q is not None else point_margin)
        y_minus_arr = low_dim_arr.copy()
        y_minus_arr[:, 1] = low_dim_arr[:, 1] - (self.anchors_radius if self.radius_q is not None else point_margin)
        if self.n_components > 2:
            z_plus_arr = low_dim_arr.copy()
            z_plus_arr[:, 2] = low_dim_arr[:, 2] + (self.anchors_radius if self.radius_q is not None else point_margin)
            z_minus_arr = low_dim_arr.copy()
            z_minus_arr[:, 2] = low_dim_arr[:, 2] - (self.anchors_radius if self.radius_q is not None else point_margin)
    ...
    @staticmethod
    def _smooth_poly_Catmull_Rom_splines():
        return 0
