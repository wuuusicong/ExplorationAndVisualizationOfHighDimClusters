...
class AMAP:
    def __init__(self, n_components=2, anchors_method='agglomerative', n_intra_anchors=20,
                 dim_reduction_algo='mds', supervised=False, k=20, self_relation=False, radius_q=0.9,
                 do_relaxation=True, top_greedy=1, magnitude_step=False, n_iter=1000, batch_size=1, stop_criteria=0.01, loss='Linf', learning_rate=None,
                 class_to_label = None,
    ...
    def fit_transform(self, X, y):
        ...
        self.anchors_indices = anchors_indices
        ...
        if self.do_relaxation:
            self._calc_inter_class_relations(y_with_centroids)
        # Dim Reduction
    ...
    def _get_intra_class_anchors(self, X, y):
        ...
        # concat clusters of centroids to clusters variable
        anchors_clusters = []
    ...
    def _dim_reduction(self, X, y, anchors_indices):
        ...
        elif self.dim_reduction_algo == 't-sne-mds':
            dim_reduction_algo_inst = TSNE(init=MDS(n_components=self.n_components).fit_transform(X))
        elif self.dim_reduction_algo == 'umap':
            dim_reduction_algo_inst = umap.UMAP(min_dist=0.1, n_neighbors=100)
        ...
        self.low_dim_anchors = self.low_dim_points[anchors_indices] # TODO ORM convert all variable like anchors indices to members
    ...
    def get_top_anchors_to_relax(self):
        ...
        # TODO ORM make sure that when self relation is true you want to zero the diagonal
    ...
    def relax_anchor_cluster(self, src_anchor_index, target_anchor_index, direction, magnitude):
        ...
        if not self.magnitude_step:
            magnitude = 1  # TODO ORM validate the learning rate is valid if magnitude step is false
    ...
    def relaxation(self):
        for i in range(self.n_iter):
            ...
            for j in range(self.batch_size):
                ...
                # TODO ORM implement vectorize version
                for ra_i in range(len(src_anchor_indices)):
                    self.relax_anchor_cluster(src_anchor_indices[ra_i], target_anchor_indices[ra_i], directions[ra_i], magnitudes[ra_i])
                # src_anchor_index, target_anchor_index, direction = self.get_top_anchors_to_relax()
                # self.print_verbose(f'Updating anchors {src_anchor_index} => {target_anchor_index} in direction {direction}')
                # self.relax_anchor_cluster(src_anchor_index, target_anchor_index, direction)
            ...
            if self.verbose:
                ...
                df['anchor'] = False
                df.loc[df.index.isin(self.anchors_indices), 'anchor'] = True
                ...
                df['label'] = df['label'].transform(lambda x: f'label_{x}')
                filled_markers = ('o', 'v', '^', '<', '>', '8', 's', 'p', '*', 'h', 'H', 'D', 'd', 'P', 'X')
                # sns.scatterplot(data=df[df['anchor']==False], x='x', y='y', hue='label', style='clusters', ax=ax,
                #                 alpha=0.3, legend=False, markers=filled_markers)
                # TODO ORM DBG
                sns.scatterplot(data=df[df['anchor'] == False], x='x', y='y', hue='label', style='clusters', ax=ax,
                                alpha=1, legend=False, markers=filled_markers)
                g = sns.scatterplot(data=df[df['anchor']==True], x='x', y='y', hue='label', style='clusters', ax=ax,
                ...
                sns.heatmap(np.square(self.inter_class_relations-self.inter_class_relations_low_dim), ax=ax, annot=False, square=True, cmap='Blues',
                            vmin=0, vmax=1,
                ...
                if self.batch_size == 0:  # No relaxation
                    fig.suptitle(f'{self.dim_reduction_algo}_{self.anchors_method}')
                else:
                    pass
                    # fig.suptitle(f'Updating anchors {str(self.anchor_to_label_cluster(src_anchor_index))} => {str(self.anchor_to_label_cluster(target_anchor_index))} in direction {direction}')

                if f'relaxation_images/iter{i}.png' == '':
                    fig.show()
                else:
    ...
    def vis_2d(_df, _x, _y, _color, _style, _title='', file_name=''):
        ...
        sns.scatterplot(data=_df, x=_x, y=_y, hue='legend', style=_style, ax=ax)
        ax.set_title(_title)
        # ax.set_xlim((-0.2, 0.2))
        # ax.set_ylim((-0.2, 0.2))
